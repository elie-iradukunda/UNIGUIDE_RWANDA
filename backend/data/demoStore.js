...
const users = [
  { id: 'usr-001', fullName: 'Jean Uwimana', email: 'student@uniguide.rw', password: 'password123', role: 'Student', department: 'Mechatronics', studentId: 'STU-2026-014', status: 'Active', canBorrow: true, canReserve: true, canViewReports: false },
  ...
  { id: 'usr-005', fullName: 'Iradukunda David', email: 'hod@uniguide.rw', password: 'password123', role: 'HOD', department: 'Mechatronics', studentId: 'HOD-002', status: 'Active', canBorrow: true, canReserve: true, canViewReports: true },
...
const announcements = [
  ...
  { id: 'ann-004', title: 'PLC practical booking', content: 'Automation Lab stations can be reserved through UniGuide before Friday practical sessions.', department: 'Mechatronics', authorName: 'Automation Lab', isNew: false, createdAt: now },
];
...
async function handleDemo(req, res) {
  ...
  if (reservationMatch && ['PUT', 'PATCH'].includes(method)) {
    ...
    if (!isOwnPendingCancellation && !allowed(user, ['Admin', 'HOD', 'StockManager', 'Lab Staff'])) return res.status(403).json({ message: 'Role not permitted to process reservations.' });
